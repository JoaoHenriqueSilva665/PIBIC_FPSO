...
def constraints_OptimumDesignofHollowShaft(X):
    ...
    try:
        ...
        if restric_01 >= 1736.93 and restric_02 <= 2.5165:
            return OptimumDesignofHollowShaft(X)
        else:
            return np.inf
...
def generate_valid_particle():
    while True:
        ...
        if constraints_OptimumDesignofHollowShaft(particle) < np.inf:
            return particle
...
def adjust_particle(particle, iteration_limit=100):
    for _ in range(iteration_limit):
        ...
        if constraints_OptimumDesignofHollowShaft(candidate) < np.inf:
            return candidate
...
def FPSO(Particles_dim, alpha_values, beta_values, Function):
    ...
    velocity_pass, position_pass = Pass_velocity(vMin_bound, bound_min, vMax_bound, bound_max,
                                                 Particles_dim)
...
def PSO(Particles_dim, Function):
    ...
    for it in range(max_iter):
        ...
        for i in range(Particles):
            ...
            # print(f"(ANTES POSITION) em i={i}:\n {position_i}")
            position_i[i] = (beta * position_i[i]+ velocity_i[i])
            # print(f"(DEPOIS POSITION) em i={i}:\n {position_i}\n")
            # print("##### verificação de Posição #####")
            # print("##### verificação da possibilidade de existir d, D e N viável #####")
            # position_i[i] = adjust_to_nearest_multiple(position_i[i])
...
def Plot_graphic(mean_best_cost, mean_best_cost_classic):
    ...
    plt.tight_layout()
    plt.savefig("Imagens/OptimumDesignofHollowShaft_.svg", format="svg")
    plt.show()
...
mean_best_cost, mean_best_cost_classic = run_pso_multiple_times(Particles_dim, constraints_OptimumDesignofHollowShaft)
